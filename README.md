...
## 功能特性
 ...
 - **下载历史** - 记录下载历史，方便查找
 - **离线语音转文字（ASR）** - 支持对已下载内容或本地视频/音频文件进行转写，输出 TXT/SRT/VTT 到原文件同目录
 - **独立转写入口** - 顶部菜单新增「转写」页面，可直接选择本地视频/音频文件进行转写
 - **ASR 自动补救** - 可配置下载完成后自动转写，适用于无字幕视频
 - **平衡档 ASR（medium + VAD）** - 默认使用 `ggml-medium.bin` 多语言模型 + VAD，兼顾准确率与速度
 - **ASR 模型一键下载** - 应用内一键下载 `medium + VAD` 模型并检查运行环境
...
6. **本地文件转写（可选）** - 在顶部「转写」页面选择本地视频/音频文件进行转写
